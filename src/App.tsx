...
export default function App() {
  ...
  const prepareMatchForCurrentStage = () => {
    ...
    if (Math.random() * 50 < teamRating + diff) {
      hGoalsCount += Math.floor(Math.random() * 3) + 1;
    }
    if (Math.random() * 60 < randomRival.rating - diff) {
      rGoalsCount += Math.floor(Math.random() * 2);
    }
    if (hGoalsCount === rGoalsCount) {
      if (teamRating >= randomRival.rating) {
        hGoalsCount += 1;
      } else {
        rGoalsCount += 1;
      }
  ...
  const finishMatchInstantly = () => {
    ...
    const userGoalChance = Math.min(
      Math.max(0.22 + ratingDiff * 0.015, 0.08),
      0.42
    );
    const rivalGoalChance = Math.min(
      Math.max(0.28 - ratingDiff * 0.015, 0.12),
      0.48
    );
    ...
    for (
      let min = startMinute;
      min <= 90;
      min += Math.floor(Math.random() * 6) + 4
    ) {
